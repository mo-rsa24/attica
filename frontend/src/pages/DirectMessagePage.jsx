...
const MessageBubble = ({ message, isMe, canActOnBid, onBidAction }) => {
  ...
  return (
    <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
      ...
      >
        {bid && (
          <div className={`rounded-lg border p-3 space-y-2 ${isMe ? 'bg-blue-700/60 border-blue-100' : 'bg-gray-50'}`}>
            ...
            {bid.counter_amount && <p className={`text-xs ${bidTextClass}`}>Counter: {bid.counter_amount}</p>}
            {canActOnBid && (
...
const AttachmentPreview = ({ attachment }) => {
  ...
  return (
    <div className="text-sm">
      {isImage ? (
        <img src={attachment.file} alt={attachment.original_name || 'Attachment'} className="max-h-52 rounded-lg" />
      ) : (
        <a href={attachment.file} target="_blank" rel="noreferrer" className="text-xs underline flex items-center gap-2">
          <FaPaperclip />
...
export default function DirectMessagePage() {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      if (payload.type === 'message') {
        ...
        setMessages((prev) => prev.map((m) => (m.id === payload.message_id ? { ...m, read_at: payload.read_at } : m)));
      }
    };
    socket.onclose = () => {
  ...
  const renderMessage = (message) => {
    ...
    return <MessageBubble key={message.id} message={message} isMe={isMe} canActOnBid={canActOnBid} onBidAction={handleBidAction} />;
  };
        ...
        <div className="bg-white rounded-xl shadow border flex flex-col h-[75vh]">
          ...
          <form onSubmit={sendMessage} className="border-t bg-white p-3">
            <div className="flex items-center gap-2">
              ...
              <label className="flex items-center gap-1 px-3 py-2 rounded-lg border cursor-pointer hover:bg-gray-50">
                <FaImage />
