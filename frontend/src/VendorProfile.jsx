...
export default function VendorProfile() {
    ...
    const startChat = async () => {
        if (!tokens?.access || !vendor || !user) return
        const res = await fetch('/api/chat/rooms/', {
            ...
            body: JSON.stringify({organizer: user.id, vendor: vendor.user_id || vendor.id})
        })
        if (res.ok) {
        ...
        <>
            ...
            {room && (
                <div className="max-w-screen-lg mx-auto mt-4 px-4">
                    <ChatRoom room={room} tokens={tokens} user={user} vendor={vendor} onClose={() => setRoom(null)}/>
                </div>
            )}
            {room && (
                <BidModal
